...
import ItemProductMainComponent from './ItemProductMainComponent';
...
const HomeComponent = () => {
  ...
  useEffect(() => {
    getProductByCategory(1, 6).then((response) => {
      setproductsValentine(response.data)
    }).catch((error) => { console.log(error) });
  }, [])
                  ...
                  <div className="row one-row">
                    {
                      flashsaleProducts.map(product =>
                        <ItemProductMainComponent key={product.id} product={product} />
                      )
                    }
                  </div>
                </div>
